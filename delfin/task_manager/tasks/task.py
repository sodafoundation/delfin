...
class StorageControllerTask(StorageResourceTask):
    ...
    def sync(self):
        ...
        :return:
        """
        LOG.info('Syncing controllers for storage id:{0}'.format(self.storage_id))
        try:
            # collect the controllers list from driver and database
            storage_controllers = self.driver_api.list_controllers(self.context,
                                                           self.storage_id)
    ...
    def remove(self):
        LOG.info('Remove controllers for storage id:{0}'.format(self.storage_id))
