...
class StoragePoolTask(StorageResourceTask):
    ...
    def sync(self):
        ...
        try:
            # collect the pools list from drivermanger and dtabase
            storage_pools = self.driver.list_pools(self.context,
                                                   self.storage_id)
            ...
            db.pools_update_all(self.context, self._get_update_list(
                storage_pools, db_pools))

            db.pools_create_all(self.context, self._get_add_list(
                storage_pools, db_pools))
        except AttributeError as e:
            LOG.error(e)
            raise exception.DolphinException(e)
