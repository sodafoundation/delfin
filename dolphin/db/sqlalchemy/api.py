...
def _storage_pool_get(context, storage_pool_id, session=None):
    ...
    if not result:
        raise exception.StoragePoolNotFound(id=storage_pool_id)
...
def storage_pools_delete(context, storage_pools_id_list):
    ...
    with session.begin():
        for storage_pool_id in storage_pools_id_list:
            ...
            if not result:
                LOG.error(exception.StoragePoolNotFound(id=storage_pool_id))
...
def storage_pools_update(context, storage_pools):
    ...
    with session.begin():
        ...
        for storage_pool in storage_pools:
            ...
            if not result:
                LOG.error(exception.StoragePoolNotFound(id=storage_pool.get(
                    'id')))
            else:
